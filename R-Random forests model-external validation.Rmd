...
```{r}
library(xlsx)
star<- read.csv("test_set_with_predictions-Random forests model-external validation.csv")
...
```{r}
tiff(filename="Decision_curve_Random forest-external validation.tif", width=600*6, height=600*6, res=90*6)
par(mfrow=c(1,1), mar=c(4.2,4.2,3,.5), pty="m")
plot_decision_curve(modelt,curve.name=c('Random Forest (External Validation)'),xlim=c(0,0.8),ylim=c(0,0.4),
...
```{r}
library(tidyverse)
tiff(filename="zhifangtu-random forests-external validation.tif", width=800*6, height=400*6, res=90*6)
...
tiff(filename="Calibration_curve_Random Forest-external validation.tif", width=600*6, height=600*6, res=90*6)
...
title("Random Forest (External Validation)", adj=0.5)
dev.off()
```
...
tiff(filename="volin plot-Random forests-external validation.tif", width=600*6, height=500*6, res=90*6)
par(mfrow = c(1,1), pty='s', mar=c(4,3,4,1),cex=1, font =1, col=1)
